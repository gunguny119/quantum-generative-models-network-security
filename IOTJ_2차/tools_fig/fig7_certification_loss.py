...
from _style import plt, COL1, COL2, ROOT, TSIRELSON, save, legend_figure
...
fig, (axT, axB) = plt.subplots(2, 1, figsize=(COL1, 3.75), sharex=True,
                               gridspec_kw=dict(height_ratios=[1.0, 0.85], hspace=0.16))
...
axB.annotate("Delft operating point\n%+.4f bits (%.1f%%)" % (dH[i245], rel[i245]),
             xy=(DELFT_CF, dH[i245]), xytext=(0.0, 0.0252), fontsize=5.8,
...
axB2.set_ylabel(r"relative gap $\Delta H / H^{S}$  (%)", fontsize=7)
...
legend_figure([
    ...
    dict(label=r"accounting gap $\Delta H$", color="#c2453f", patch=True, alpha=0.16),
   ...
   note="dotted vertical line: Delft operating point CF = 0.2106.")
for i in o:
    print("   V=%.3f CF=%.6f h_S=%.6f h_full=%.6f dH=%+.6f rel=%.2f%%"
          % (kept[i][0]["V"], cf[i], hS[i], hF[i], dH[i], rel[i]))
