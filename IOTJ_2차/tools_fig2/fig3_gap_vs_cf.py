...
from _style2 import plt, COL2, ROOT, TSIRELSON, save, legend_figure
...
fig, (axL, axR) = plt.subplots(1, 2, figsize=(COL2, 2.95))
...
legend_figure([
    ...
    dict(label="parameter-matched (expressivity confound)", color="#8a8a8a", marker="D", ls="-.", ms=2.8, lw=0.7),
    dict(label="closed form  max(0, 2$\\lambda$-1)", color="#1b3a5c", marker="None", ls="-"),
    dict(label="LP solution", color="#c2453f", marker="o", ls="none", mfc="white"),
], NAME + "_legend", ncol=3,
   note=("shaded band: $\\pm$1 SD over 5 seeds (stored for the two gap series only).  "
         "dotted line: Tsirelson CF = 0.4142.  "
         r"max $|$LP $-$ closed form$|$ = %.1e" % d["self_check_maxdiff"]))
