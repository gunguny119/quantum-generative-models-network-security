...
FIGDIR = os.path.join(ROOT, "figures2")
...
def save(fig, name):
    ...
    print("[saved] figures2/%s.{pdf,png}" % name)
...
def legend_figure(entries, name, ncol=None, note=None, width=None):
    ...
    fig.legend(handles, [e["label"] for e in entries], loc="upper center", ncol=ncol,
               ...
               fontsize=7, borderaxespad=0.0, bbox_to_anchor=(0.5, y0))
    if note:
    ...
    print("[saved] figures2/%s.{pdf,png}  (bottom legend attached)" % base)
