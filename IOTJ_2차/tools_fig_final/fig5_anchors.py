"""Fig 5 (final) — anchors on the curve (left) and the off anchor (right).
...
print("[filter] left: 2 anchors + %d reference rows;  right: %d cells (a:%d, b:%d)"
      % (len(rs), len(cells), len(da), len(db)))

fig, (axL, axR) = plt.subplots(1, 2, figsize=(COL2, 3.2),
                               gridspec_kw=dict(width_ratios=[1.0, 1.0], wspace=0.42))
...
for key, mk, col, src in (("kcbs", "o", "#c2453f", "track_A_kcbs"),
    ...
    print("   L %-6s CF=%.4f gap=%.6f dev=%.2f%%" % (key, cf, gap, 100 * A[key]["rel_dev"]))
...
for grp, xc, mk, col, lab in ((da, 0, "v", "#4a4a4a", "definition a"),
    ...
    print("   R %-13s n=%d  gap_upper max=%.3e" % (lab, len(y), max(y)))
axR.set_yscale("log")
